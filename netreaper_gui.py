...
class CommandThread(QThread):
    ...
    def terminate(self) -> None:
        """Terminate the underlying process if it is running."""
        try:
...
class WirelessTab(CategoryTab):
    ...
    def refresh_interfaces(self) -> None:
        ...
        Attempts to use `iw dev` to list Wi‑Fi interfaces; if none are detected,
        falls back to enumerating all network interfaces under `/sys/class/net` and
        excludes the loopback device. This improves compatibility on systems
        lacking `iw` or where Wi‑Fi devices are not reported by it.
        """
        # First attempt to list wireless interfaces via iw
    ...
    def deauth_attack(self) -> None:
        ...
        # Record BSSID in history if provided
    ...
    def capture_handshake(self) -> None:
        ...
        if self.main_window:
            self.main_window.add_bssid_history(target)
        channel = self.channel_input.text().strip()
        # Use short flag '-c' for channel (not '--channel') to be compatible with airodump-ng
        channel_arg = f"-c {quote(channel)}" if channel else ""
...
class NetReaperGui(QWidget):
    ...
    def __init__(self):
        ...
        # BSSID fields and history for wireless operations
        ...
        # Add a Stop button to terminate all active tasks
        ...
        def toggle_checked(checked: bool, action: QAction=lite_action) -> None:
            self.toggle_lite_mode(checked)
        lite_action.triggered.connect(toggle_checked)
        ...
        # Apply loaded history to existing fields
    ...
    def register_target_field(self, field: TargetField) -> None:
        ...
        if field not in self.target_fields:
            ...
            # When registering a new field, initialise its history with the global list
            if self.target_history:
                field.set_history(self.target_history)
            else:
                # Use current value as a starting entry if no history exists yet
                current = field.value()
                if current:
    ...
    def register_bssid_field(self, field: TargetField) -> None:
        """Register a BSSID field to share a common history across wireless forms."""
        if not field.share_history:
            return
    ...
    def add_bssid_history(self, bssid: str) -> None:
        """Add a BSSID to the shared wireless history and update all fields."""
        if not bssid:
            return
    ...
    def _load_target_history(self) -> None:
        """Populate self.target_history from the user's history file if present."""
        # Attempt to read scan history from NetReaper's history folder
        # This file stores the last scan targets and is optional
        history_paths = [
            ...
            if os.path.exists(path):
                try:
                    with open(path, "r", encoding="utf-8") as f:
                        for line in f:
                            # Split by whitespace and take the first token as the target
    ...
    def execute_command(self, command: str, description: str, target: Optional[str] = None) -> None:
        """Execute a shell command with optional target context.

        Before executing, perform a basic availability check on the underlying tool
        and prefix the command with `NR_LITE_MODE=1` if Lite mode is enabled.
        """
        cmd_tokens = shlex.split(command)
        # Determine the actual tool being called (skip sudo if present)
        tool = cmd_tokens[1] if cmd_tokens and cmd_tokens[0] == "sudo" and len(cmd_tokens) > 1 else cmd_tokens[0] if cmd_tokens else None
        if tool:
            if shutil.which(tool) is None:
                QMessageBox.warning(self, "Tool missing", f"The command '{tool}' is not available in PATH. Cannot run '{description}'.")
                return
        # Prepend NR_LITE_MODE if enabled
        actual_command = command
        if self.lite_mode and not command.startswith("NR_LITE_MODE="):
            actual_command = f"NR_LITE_MODE=1 {command}"
        self.output_log.appendPlainText(f"[{description}] $ {actual_command}")
        if target:
            self.add_target_history(target)
        # Launch thread to execute the command
        thread = CommandThread(actual_command)
        ...
        # Record command in history list for re-run capability
        item = QListWidgetItem(f"{description}: {actual_command}")
    ...
    def stop_all_tasks(self) -> None:
        ...
        for thread in list(self.active_threads):
            try:
                ...
                pass
        # Clear list; threads will self-remove on finished
